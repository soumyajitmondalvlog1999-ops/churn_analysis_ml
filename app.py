...
import pandas as pd
import numpy as np  # Make sure to add numpy
import joblib
...
# (You should add all 50+ states for a complete app)
STATE_OPTIONS = ['KS', 'OH', 'NJ', 'GA', 'VT', 'MD', 'IN', 'NY', 'LA', 'AZ', 'MO', 'VA', 'OR', 'UT', 'WY', 'MI', 'ID', 'MT', 'MN', 'AL', 'WA', 'TX', 'RI', 'WI', 'CO', 'NV', 'MS', 'WV', 'KY', 'AR', 'DC', 'CT', 'HI', 'ME', 'NE', 'NM', 'NH', 'NC', 'SC', 'TN', 'OK', 'FL', 'IA', 'PA', 'SD', 'MA', 'CA', 'DE', 'IL', 'AK', 'ND']
PLAN_OPTIONS = ['no', 'yes']
...
# --- NEW SECTION: FEATURE IMPORTANCE ---
...
try:
    ...
    # get_feature_names_out() is the correct method
    ...
    st.info("""
    This chart shows the most important factors the model uses to predict churn.
    A higher bar means that feature has a bigger impact on the prediction.
    """, icon="ℹ️")
    
    # Display the bar chart (top 15)
    st.bar_chart(imp_df.head(15))

    with st.expander("Show all feature importances (raw data)"):
        st.dataframe(imp_df)
