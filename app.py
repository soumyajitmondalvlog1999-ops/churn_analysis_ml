...
def build_model(df, target_column, problem_type, ignore_cols):
    ...
    # --- 10. FEATURE IMPORTANCE SECTION (MODIFIED) ---
    ...
    try:
        ...
        # --- NEW: Create Percentage Column ---
        imp_df['Importance (%)'] = (imp_df['Importance'] * 100).round(2)
        
        # Create chart_df for plotting (using Feature as index)
        chart_df = imp_df.set_index('Feature')

        st.info(f"""
        This chart shows the importance of each feature as a **percentage**. 
        Higher percentages mean the feature had a bigger impact on the outcome.
        
        **Top 3 Factors:**
        1. {imp_df.iloc[0]['Feature']}
        2. {imp_df.iloc[1]['Feature']}
        3. {imp_df.iloc[2]['Feature']}
        """, icon="ℹ️")

        # Display the bar chart (plotting the new percentage column)
        st.bar_chart(chart_df['Importance (%)'].head(15))
        
        # Add an expander for the full list with both columns
        with st.expander("Show all feature importances (raw and percentage)"):
            st.dataframe(imp_df) # This df has 'Feature', 'Importance', and 'Importance (%)'
